...
// PW Mode (1x1 / Pointwise)
    ...
    always_comb begin
            ...
            S_STREAM_AND_ARITHMETIC: begin
                ...
                if (pw_mode_run) begin
                    ...
                    if (ctx_issue < num_ctx_cur) begin
                        ...
                        if (!dw_mode_run) begin
                            // Channel-major stream: pixel advances fastest, then channel.
                            ...
                            // Depthwise mode: channel is fixed per lane, only pixel advances
                ...
                if (weight_ok_stream) begin
                    ...
                    if (!pw_mode_run) begin
                        if (!dw_mode_run) begin
                            // Channel-major order: pixel advances fastest, then channel.
        ...
        // Hard reset on layer start to avoid stale waddr/ctx between layers.
        if (start) begin
            state_n        = S_SCAN;
            win_cnt_n      = '0;
            grp_idx_n      = '0;
...
`ifndef SYNTHESIS
    // Debug: PE(0,0) inputs (top-left lane/col) per valid stream beat.
    integer pe00_dbg_count;
    always_ff @(posedge clk) begin
        if (reset || start) begin
            pe00_dbg_count <= 0;
        end else if (stream_valid && (pe00_dbg_count < 27)) begin
            $display("PE00 dbg: ctx=%0d pix_sel=%0d chan_sel=%0d act=%0d wgt=%0d",
                             ctx_out, pix_sel, chan_sel, ifmap_array[0], weight_array[0]);
            pe00_dbg_count <= pe00_dbg_count + 1;
        end
    end
`endif
