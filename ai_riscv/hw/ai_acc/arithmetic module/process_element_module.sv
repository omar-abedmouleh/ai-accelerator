...
module process_element_module #(
  ...
  // Output zero-point: change it if SW output offset differs from -128. Mobilnet V1 uses -128.
  ...
  // register final MAC result and params at mac_last
  ...
  // 32x32 multiply idea derived from vector_mul.sv and vproc_mul_block.sv.
  // Those files are licensed under the Solderpad Hardware License v2.1
  
  
  logic signed [31:0] A_s;
  logic signed [31:0] B_s;
  logic        [5:0]  left_shift_b;
  ...
  always_comb begin
    left_shift_b = (shift_win_q > 0) ? shift_win_q[5:0] : 6'd0;
    if (left_shift_b > 6'd31)
      left_shift_b = 6'd31;

    A_s = $signed(acc_win_q);
    if (left_shift_b != 6'd0)
      A_s = $signed(A_s <<< left_shift_b);
  ...
  // Stage C: Requantization (TFLite SRDHM + RDPOT), uses mult_prod_b
  
  logic signed [5:0]        shift_amt_b;
  logic        [5:0]        right_shift_b;
  logic signed [31:0]       srdhm_b;
  logic signed [31:0]       rdp_b;
  ...
  always_comb begin
    shift_amt_b  = shift_q;
    right_shift_b = (shift_amt_b < 0) ? (-shift_amt_b[5:0]) : 6'd0;
    if (right_shift_b > 6'd31)
      right_shift_b = 6'd31;

    if ((acc_win_q == MIN_NEG_32) && (mult_win_q == MIN_NEG_32)) begin
      srdhm_b = MAX_POS_32;
    end else begin
      if (mult_prod_b >= 0)
        srdhm_b = $signed((mult_prod_b + 64'sd1073741824) >>> 31);
      else
        srdhm_b = $signed((mult_prod_b - 64'sd1073741824) >>> 31);
    end

    if (right_shift_b == 0) begin
      rdp_b = srdhm_b;
    end else begin
      if (srdhm_b >= 0)
        rdp_b = $signed((srdhm_b + (32'sd1 <<< (right_shift_b - 1))) >>> right_shift_b);
      else
        rdp_b = $signed((srdhm_b + ((32'sd1 <<< (right_shift_b - 1)) - 32'sd1)) >>> right_shift_b);
    end

    acc_with_offset_b = rdp_b + SAT_ACC_W'(OUTPUT_OFFSET_CONST);
