...
module arithmetic_module #(
  ...
  // 1) Robuste Puls-Erzeugung für externe done-signale
  logic conv_done_d;
  logic pool_done_d;

  logic conv_done_pulse;
  logic pool_done_pulse;
  ...
  always_ff @(posedge clk or posedge reset) begin
    if (reset) begin
      ...
      conv_done_d <= conv_done_global;
      pool_done_d <= pooling_done_ext;
    end
  end

  assign conv_done_pulse = conv_done_global & ~conv_done_d;
  assign pool_done_pulse = pooling_done_ext  & ~pool_done_d;

  // Pooling im DW-Modus erstmal aus (robuster Default)
  logic pool_on_to_ctrl;
  assign pool_on_to_ctrl = pooling_on_cfg & ~dw_on_cfg;
  ...
  arithmetic_controller u_ctrl (
    ...
    .relu_on         (relu_on_cfg),
    .pool_on         (pool_on_to_ctrl),

    .arithmetic_done (o_done_arithmetic),
    .conv_done_global(conv_done_pulse),
    .relu_done       (relu_done_int),
    .pooling_done    (pool_done_pulse),

    .arithmetic_en        (ctrl_arithmetic_en),
    .relu_en              (ctrl_relu_en),
    .pooling_en           (ctrl_pooling_en),
