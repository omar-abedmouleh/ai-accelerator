...
IMG_W = 224          # Original Bildgröße (ungepadet)
IMG_H = 224
PAD_W = IMG_W + 2    # 226
PAD_H = IMG_H + 2
CH    = 3
...
PATH_HW_FM = r"C:\Users\amora\Desktop\Neuer Ordner (2)\ai_riscv\hw\ai_acc\tb\layer0_avgpool_ofmap_hwc.bin"
...
def requant_and_saturate(acc32: int, shift: int) -> np.int8:
    ...
    if shift_amt == 0:
        ...
        acc_shifted = acc_round >> shift_amt  # arithmetischer Shift
...
x = x_flat.reshape(PAD_H, PAD_W, CH)  # (226,226,3) HWC
...
for oy in range(FM_H):
    for ox in range(FM_W):
        ...
        for k in range(CO):
            w_k_i32 = w_arr[k].astype(np.int32)
...
# 5) 2x2 Average-Pooling mit Stride 2 (HW-Verhalten nachbauen)
#    RTL: sum_ext = v00+v01+v10+v11; pooled = sum_ext >>> 2
#    => Floor Richtung -∞ (arithmetischer Shift)
...
for oy in range(POOL_H):
    for ox in range(POOL_W):
        ...
        window = y_conv[in_y:in_y+POOL_K, in_x:in_x+POOL_K, :]  # (2,2,CO)

        # explizite Summe und arithmetischer Shift wie im RTL
        v00 = window[0, 0, :].astype(np.int32)
        v01 = window[0, 1, :].astype(np.int32)
        v10 = window[1, 0, :].astype(np.int32)
        v11 = window[1, 1, :].astype(np.int32)

        sum_int = v00 + v01 + v10 + v11    # [-512..508]
        avg_int = sum_int >> 2             # arithmetischer Shift

        y_sw[oy, ox, :] = avg_int.astype(np.int8)

print("Software-Avgpool fertig. Shape:", y_sw.shape)
...
y_hw = fm_hw_flat.reshape(POOL_H, POOL_W, CO)
print("HW-Featuremap (Avgpool) geladen. Shape:", y_hw.shape)
...
if num_mismatch > 0:
    ...
    print("Perfekt: HW und SW (mit Avgpool) sind bitgenau identisch!")
