...
module line_buffer_3x3 #(
    ...
    input  logic [1:0]  cfg_stride,  // stride >= 1 (2-bit)
    ...
    always_comb begin
        ...
        stride_eff = (cfg_stride == 2'd0) ? 16'd1 : {14'd0, cfg_stride};
        ...
        if (reset) begin
            ...
            if (px_valid) begin
                ...
                if ((row >= (WIN_SIZE-1)) && (col >= (WIN_SIZE-1))) begin
                    if ((x_phase == 16'd0) && (y_phase == 16'd0)) begin
                        win_valid <= 1'b1;
                        win_out <= {
                        b0_n, b1_n, b2_n,
                        m0_n, m1_n, m2_n,
