...
module processing_module #(
    ...
    // Pooling Lines (fixed 2x2 input width)
    localparam int POOL_W_MAX = 2;
    localparam int POOL_H_MAX = 2;
    logic signed [OUT_W-1:0] pool_line0 [0:COLS_MAX-1][0:POOL_W_MAX-1];
    logic signed [OUT_W-1:0] pool_line1 [0:COLS_MAX-1][0:POOL_W_MAX-1];
    ...
    logic [$clog2(POOL_H_MAX)-1:0] pool_w_y;
    logic [$clog2(POOL_W_MAX)-1:0] pool_w_x;
            ...
            if (start_frame) begin
                ...
                if ((st == ST_RUN) && pool_active) begin
                    if (cell_active &&
                        ...
                        if ((px_xi >= 0) && (px_xi < POOL_W_MAX)) begin
                            if (win_y32[0] == 1'b0)
...
// latch that we must ACK fetcher only after pooling_done
    ...
    always_comb begin
        if (pool_active) begin
            ...
            of_w_y   = {{($bits(of_w_y)-$bits(pool_w_y)){1'b0}}, pool_w_y};
            of_w_x   = {{($bits(of_w_x)-$bits(pool_w_x)){1'b0}}, pool_w_x};
            of_wdata = pool_wdata;
        end else begin
            of_we    = store_we;
            of_w_ch  = store_w_ch;
