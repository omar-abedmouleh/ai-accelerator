...
`ifdef RVFI
  ...
  logic [4:0]   vrf_waddr;
  logic [4:0]   vrf_waddr_vs1;
  logic [4:0]   vrf_waddr_vs2;

  // Vector Execute Unit signals
  // logic         vex_done; // TODO : NOT USED REMOVE
  // logic         vex_err;
  logic         vex_busy;
  logic         vex_resp_valid;
  v_alu_op_e    vex_alu_op;
  vew_e         vex_element_width;
  logic         scalar_src_operand;
  logic         immediate_src; // vector immediate select
  ...
  ibex_id_stage #(
    ...
    .vex_req_o(vex_req),
    .is_mul_o(is_mul),
    .scalar_src_operand_o(scalar_src_operand),
    .immediate_src_o (immediate_src),
  ...
  always_comb begin
    if (is_vsetvli) begin
      if (v_rs1_en) begin
        if (~result_ex[0]) result_ex_custom = alu_operand_a_ex; // alu_operand_b_ex should be vlmax
  ...
  logic refresh_vs1;
  logic refresh_vs2;
  // Vector destination register handling
  // Base vd from instruction (rd field)
  logic [4:0] vd_base;
  assign vd_base = instr_rdata_id[11:7];

  // Counter for widening multi-register writes (incremented on refresh pulses)
  logic [4:0] vd_counter_q, vd_counter_d;
  logic [4:0] vs1_counter_q, vs1_counter_d;
  logic [4:0] vs2_counter_q, vs2_counter_d;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      vd_counter_q <= vd_counter_d;
      vs1_counter_q <= vs1_counter_d;
      vs2_counter_q <= vs2_counter_d;
    end
  end


  always_comb begin
    vd_counter_d = vd_counter_q;
    vs1_counter_d = vs1_counter_q;
    vs2_counter_d = vs2_counter_q;
    // Initialize on new vector execute request start
    if ((vex_req & ~vex_busy) | (vst_req & ~vsu_busy)) begin
      vd_counter_d = vd_base;
      vs1_counter_d = instr_rdata_id[19:15]; // vs1 field
      vs2_counter_d = instr_rdata_id[24:20]; // vs2 field
    end else begin
      if (vex_refresh | st_refresh) begin
        vd_counter_d = vd_counter_q + 5'd1;
      end
      if (refresh_vs1) begin
        vs1_counter_d = vs1_counter_q + 5'd1;
      end
      if (refresh_vs2) begin
        vs2_counter_d = vs2_counter_q + 5'd1;
      end
    end
  end

  // Use counter while execute unit active, else base
  assign vrf_waddr = (~vex_busy) ? vd_base : vd_counter_q;
  assign vrf_waddr_vs1 = (~vex_busy) ? instr_rdata_id[19:15] : vs1_counter_q;
  assign vrf_waddr_vs2 = (~vex_busy) ? instr_rdata_id[24:20] : vs2_counter_q;
  ...
  always_comb begin
    if (lsu_mux) begin
      ...
      vrf_rd_vreg_a_muxed = (~vsu_busy) ? vd_base : vd_counter_q;
    end else begin
      // Vector execute unit uses both ports A and B
      vrf_rd_vreg_a_muxed = vrf_waddr_vs1; // vs1 field for vector ALU
    end
  end
  ...
  // Mux scalar register vs vector-immediate (simm5) with proper sign-extension
  logic [31:0] rf_rdata_a_vec_mux;
  logic [4:0]  vi_imm5;
  logic [31:0] vi_imm5_sext32;
  assign vi_imm5         = instr_rdata_id[19:15];
  assign vi_imm5_sext32  = {{27{vi_imm5[4]}}, vi_imm5};
  assign rf_rdata_a_vec_mux = immediate_src ? vi_imm5_sext32 : rf_rdata_a;
  ...
  vector_ex_unit #(
    ...
    .is_mul(is_mul),                // Multiplication operation flag
    .op_i(vex_alu_op),              // ALU operation type
    .vew_i(vex_element_width),      // Element width
    .v_vl_i(v_vl),                  // Vector length
    .rf_rdata_a_i(rf_rdata_a_vec_mux),
    .is_scalar_i(scalar_src_operand),
    // Vector register file interface
    .vrf_rdata1_i(vrf_rd_rdata_a),
    .vrf_rdata2_i(vrf_rd_rdata_b),
    // Vector register file interface - write
    .vrf_we_o(vex_vrf_we),
    .vrf_wdata_o(vex_vrf_wdata),
    .resp_valid_o(vex_resp_valid),
    // Status signals
    .busy_o(vex_busy),
    .refresh_vd_o(vex_refresh),
    .refresh_vs1_o(refresh_vs1), // NEU
    .refresh_vs2_o(refresh_vs2)  // NEU
  );
