...
module vector_mul #(
        parameter int unsigned        MUL_OP_W       = 32,   // MUL unit operand width in bits
        ...
        input  logic                  pipe_out_ready_i, // immer auf 1
        output CTRL_T                 pipe_out_ctrl_o,
        output logic [MUL_OP_W  -1:0] pipe_out_res_o,
        // High part of result for widening operations (e.g. VWMUL)
        output logic [MUL_OP_W  -1:0] pipe_out_res_h_o
    ...
    logic  state_ex1_ready,                      state_ex2_ready,   state_ex3_ready,   state_res_ready; // always 1
    ...
    logic [MUL_OP_W  -1:0] result_h_q,     result_h_d;
    ...
    generate
        if (BUF_OPERANDS) begin : gen_buf_operands
        ...
        end else begin : gen_no_buf_operands
            always_comb begin
                state_ex1_valid_q = state_ex1_valid_d;
                state_ex1_q       = state_ex1_d;
                operand1_q        = operand1_d;
        ...
        if (BUF_MUL_IN) begin : gen_buf_mul_in
            ...
            assign state_ex2_ready = ~state_ex2_valid_q | state_ex3_ready; // immer auf 1
        end else begin : gen_no_buf_mul_in
            always_comb begin
                state_ex2_valid_q = state_ex1_valid_q;
                state_ex2_q       = state_ex1_q;
                accumulator2_q    = accumulator2_d;
        ...
        if (BUF_MUL_OUT) begin : gen_buf_mul_out
            always_ff @(posedge clk_i or negedge async_rst_ni) begin : vproc_mul_stage_ex3_valid
                ...
                else if (state_ex3_ready) begin // immer auf 1
                    state_ex3_valid_q <= state_ex2_valid_q;
                end
            end
            always_ff @(posedge clk_i) begin : vproc_mul_stage_ex3
                if (state_ex3_ready & state_ex2_valid_q) begin // nur von state_ex2_valid_q abhängig
                    state_ex3_q    <= state_ex2_q;
                    // result_mask2_q <= result_mask2_d;
                end
            end
            assign state_ex3_ready = ~state_ex3_valid_q | state_res_ready; // immer auf 1
        end else begin : gen_no_buf_mul_out
            always_comb begin
                state_ex3_valid_q = state_ex2_valid_q;
                state_ex3_q       = state_ex2_q;
                // result_mask2_q    = result_mask2_d;
            end
            assign state_ex3_ready = state_res_ready; // immer auf 1
        end

        if (BUF_RESULTS) begin : gen_buf_results
            always_ff @(posedge clk_i or negedge async_rst_ni) begin : vproc_mul_stage_res_valid
                ...
                else if (state_res_ready) begin // immer auf 1
                    state_res_valid_q <= state_ex3_valid_q;
                end
            end
            always_ff @(posedge clk_i) begin : vproc_mul_stage_res
                if (state_res_ready & state_ex3_valid_q) begin // nur von state_ex3_valid_q abhängig
                    state_res_q    <= state_ex3_q;
                    result_q       <= result_d;
                    result_h_q     <= result_h_d;
                    // result_mask3_q <= result_mask3_d;
                end
            end
            assign state_res_ready = ~state_res_valid_q | pipe_out_ready_i; // immer auf 1
        end else begin : gen_no_buf_results
            always_comb begin
                state_res_valid_q = state_ex3_valid_q;
                state_res_q       = state_ex3_q;
                result_q          = result_d;
            ...
            assign state_res_ready = pipe_out_ready_i; // immer auf 1
        end
    endgenerate
    ...
    assign pipe_out_res_h_o = result_h_q;
    ...
    always_comb begin
        ...
        for (int i = 0; i < MUL_OP_W/8; i++) begin
            op1_signs[i] = state_ex1_q.mul_mode.op1_signed & operand1_q[8*i+7]; // always signed
            op2_signs[i] = state_ex1_q.mul_mode.op2_signed & operand2_q[8*i+7]; // always signed
        end
    end
    ...
    always_comb begin
        ...
        unique case (state_ex1_q.eew)
            EW8:  ex1_vsew_8 = 1'b1;
            EW16: ex1_vsew_8 = 1'b0;
            EW32: ex1_vsew_8 = 1'b0;
            default: ;
        endcase
        unique case (state_ex1_q.eew)
            EW8:  ex1_vsew_32 = 1'b0;
            EW16: ex1_vsew_32 = 1'b0;
            EW32: ex1_vsew_32 = 1'b1;
            default: ;
        endcase
    end
    ...
    always_comb begin
        ...
        for (int i = 0; i < MUL_OP_W / 32; i++) begin
            mul_op2[68*i +: 68] = {
                // EW8: byte 3, EW32: lower halfword
                ...
                // EW8: byte 1, EW32: upper halfword
    ...
    always_comb begin
        ...
        unique case (state_ex1_q.mul_mode.op)
            MUL_VSMUL: begin
                unique case (state_ex1_q.eew)
                    EW8:  accumulator2_d = {MUL_OP_W/32{32'h40404040}};
                    EW16: accumulator2_d = {MUL_OP_W/32{32'h40004000}};
                    EW32: accumulator2_d = {MUL_OP_W/32{32'h40000000}};
                    default: ;
                endcase
            end
    ...
    always_comb begin
        ...
        unique case (state_ex2_q.eew)
            EW8:  ex2_vsew_8 = 1'b1;
            EW16: ex2_vsew_8 = 1'b0;
            EW32: ex2_vsew_8 = 1'b0;
            default: ;
        endcase
        unique case (state_ex2_q.eew)
            EW8:  ex2_vsew_16 = 1'b0;
            EW16: ex2_vsew_16 = 1'b1;
            EW32: ex2_vsew_16 = 1'b0;
            default: ;
        endcase
        unique case (state_ex2_q.eew)
            EW8:  ex2_vsew_32 = 1'b0;
            EW16: ex2_vsew_32 = 1'b0;
            EW32: ex2_vsew_32 = 1'b1;
            default: ;
        endcase
    end
    ...
    // accumulator flags
    logic mul_accflag, mul_accsub;
    always_comb begin
        mul_accflag = DONT_CARE_ZERO ? '0 : 'x;
        unique case (state_ex2_q.mul_mode.op)
            MUL_VMUL:  mul_accflag = 1'b0;
            VWMUL:     mul_accflag = 1'b0;
    ...
    always_comb begin
        ...
        result_h_d = {MUL_OP_W{1'b0}}; // default
        unique case (state_ex3_q.mul_mode.op)
            ...
            // Widening multiply: produce both low and high parts
            VWMUL: begin
                unique case (state_ex3_q.eew)
                    // 8-bit elements widen to 16-bit; pack two 16-bit into each 32-bit word
                    EW8: begin
                        // lower 32 bits: bytes 0 and 1
                        for (int j = 0; j < (MUL_OP_W / 16); j++) begin
                            result_d  [16*j +: 16] = mul_res[33*j       +: 16];
                            result_h_d[16*j +: 16] = mul_res[33*(j + 2) +: 16];
                        end
                    end
                    // 16-bit elements widen to 32-bit; one result per halfword
                    EW16: begin
                        // j=0 -> low halfword into result_d, j=1 -> high halfword into result_h_d
                        for (int j = 0; j < (MUL_OP_W / 16); j++) begin
                            if (j == 0) begin
                                result_d[31:16] = mul_res[66*j+16 +: 16];
                                result_d[15:0 ] = mul_res[66*j    +: 16];
                            end else begin
                                result_h_d[31:16] = mul_res[66*j+16 +: 16];
                                result_h_d[15:0 ] = mul_res[66*j    +: 16];
                            end
                        end
                    end
                    EW32: begin
                        for (int i = 0; i < (MUL_OP_W / 32); i++) begin
                            result_d  [32*i +: 32] = res32[64*i +: 32];       // low
                            result_h_d[32*i +: 32] = res32[64*i+32 +: 32];     // high
                        end
                    end
                    default: ;
                endcase
